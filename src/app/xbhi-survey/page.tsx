...
const Survey = () => {
  ...
  const calculateScore = (updatedResponses = responses) => {
    ...
    Object.keys(weights).forEach((key) => {
      ...
      maxPossibleScore += 10 * weight; // Max rating of 10 per question
    });
  ...
  return (
    ...
    <div className="max-w-3xl mx-auto p-6 bg-white dark:bg-gray-800 shadow-md rounded-lg">
      ...
      <div className="space-y-4">
        {[
          { name: 'recognition', label: `How well do you recognize ${brandName} compared to others in the industry?` },
          { name: 'trust', label: `How would you rate your level of trust in ${brandName} products/services?` },
          { name: 'relevance', label: `How relevant do you feel ${brandName} is to your needs and preferences?` },
          { name: 'recommendation', label: `How likely are you to recommend ${brandName} to others?` },
          { name: 'reputation', label: `How would you rate ${brandName}'s reputation compared to its competitors?` },
          { name: 'innovation', label: `How innovative do you perceive ${brandName} to be?` },
          { name: 'resonance', label: `How well does ${brandName}'s messaging resonate with you?` },
          { name: 'satisfaction', label: `How do you feel about your overall experience with ${brandName}?` },
          { name: 'delivery', label: `How well does ${brandName} deliver on its promises?` },
        ].map((question, index) => (
